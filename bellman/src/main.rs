...
impl<'a, E: Engine> Circuit<E> for &'a CircuitImpl<E::Fr> {
    fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        let public: Vec<Assignment<E>> = self.public_witness.clone().into_iter()
            .map(|value| Assignment {
                variable: cs.alloc_input(|| "aux", || Ok(value)).unwrap(), 
                constrained: false, 
                value: value
            }).collect();
...
It requires the R1CS (A, B, C matrices) as well as assignments for public and auxiliary input.

The witnesses are consumed from a comma separated list of field elements.
...
The index (row) of a public input i_n is |aux| + n. The entire witness is prepended with 1 in 0th position.

By applying the witness, the matrices are expected to satisfy (A * B) + C = 0

Usage:
    {} <public witness> <aux witness> <A matrix> <B matrix> <C matrix>", args[0])
}
...
fn main() {
    ...
    let public = parse_witness(&args[1]);
    let aux = parse_witness(&args[2]);
    let c = CircuitImpl {
        public_witness: public.clone(),
        aux_witness: aux,
        qap_a: args[3].clone(),
        qap_b: args[4].clone(),
        qap_c: args[5].clone(),
    ...
    match verify_proof(&pvk, &proof, &public) {
        Ok(v) => assert!(v),
